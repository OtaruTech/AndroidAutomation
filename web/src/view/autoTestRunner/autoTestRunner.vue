  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="测试用例" prop="testcases" width="400" />
        ...
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="text" icon="el-icon-edit" size="small" class="table-button" @click="updateTestRunner(scope.row)">变更</el-button>
...
export default {
  ...
  methods: {
    ...
    async onDelete() {
      ...
      const res = await deleteTestRunnerByIds({ ids })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
    ...
    async updateTestRunner(row) {
      const res = await findTestRunner({ ID: row.ID })
      this.type = 'update'
      if (res.code === 0) {
        this.formData = res.data.retestRunner
    ...
    async deleteTestRunner(row) {
      const res = await deleteTestRunner({ ID: row.ID })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
    ...
    async enterDialog() {
      ...
      switch (this.type) {
        case 'create':
          res = await createTestRunner(this.formData)
          break
        case 'update':
          res = await updateTestRunner(this.formData)
          break
        default:
          res = await createTestRunner(this.formData)
          break
